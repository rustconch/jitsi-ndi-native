...
if (!(Test-Path (Join-Path $ProjectRoot "src")) -or !(Test-Path (Join-Path $ProjectRoot "CMakeLists.txt"))) {
    throw "Project root not found. Run this script from D:\MEDIA\Desktop\jitsi-ndi-native or from the extracted patch folder inside it."
}
...
foreach ($Rel in $Files) {
    ...
    if (!(Test-Path $PatchFile)) {
        throw "Patch file not found: $Rel"
    }
...
Write-Host ""
Write-Host "Done. Backup folder: $BackupDir"
Write-Host "Now build:"
Write-Host "  cd $ProjectRoot"
Write-Host "  cmake --build build --config Release"
Write-Host ""
Write-Host "If CMake asks for reconfigure, run:"
