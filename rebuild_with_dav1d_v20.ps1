...
function Info($m) { Write-Host "[v20] $m" -ForegroundColor Cyan }
function Warn($m) { Write-Host "[v20][WARN] $m" -ForegroundColor Yellow }
function Die($m) { Write-Host "[v20][ERROR] $m" -ForegroundColor Red; exit 1 }
...
$copyScript = Join-Path $ScriptDir 'copy_runtime_dlls_v20.ps1'
$checkScript = Join-Path $ScriptDir 'check_dav1d_runtime_v20.ps1'
