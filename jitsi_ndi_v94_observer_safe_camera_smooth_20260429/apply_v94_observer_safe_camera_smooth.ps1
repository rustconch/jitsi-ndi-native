...
$backupDir = Join-Path $repoRoot ("backup_v94_observer_safe_camera_smooth_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
...
Write-Host "v94 applied. Backup:" $backupDir
Write-Host "Changes: observer-safe constraints, no legacy LastNChangedEvent, silent/muted conference request, camera stale-queue drop, faster camera-local AV1 reset, dav1d OBU warning filter."
