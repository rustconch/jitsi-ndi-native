...
std::string makeReceiverVideoConstraintsMessage(
    ...
    /*
        v85 live-safe selected mode:
        - Camera sources keep the 1080p ceiling.
        - Desktop/screen-share sources are still requested as separate NDI sources, but at
          30fps like cameras; only muted-source pruning and live-queue protection are used
          overloaded the single media path while JVB reported packetLoss=0.
    ...
    */
    ...
    for (std::size_t i = 0; i < realSources.size(); ++i) {
        ...
        const double sourceMaxFrameRate = 30.0; // v85a: do not reduce screen-share FPS; keep all active video at 30fps.
    ...
    */
    ...
    const auto now = std::chrono::steady_clock::now();
    {
        ...
        if (haveLastConstraints && constraintsMessage == lastConstraintsMessage) {
            ...
            if (sinceMs < 5000) {
                Logger::info(
                    "NativeWebRTCAnswerer: v85 skipped duplicate 1080p ReceiverVideoConstraints, reason=",
                    reason,
                    " sinceMs=",
                    sinceMs
                );
    ...
    Logger::info(
        "NativeWebRTCAnswerer: requesting v85a live-safe selected-only 1080p/30fps constraints for camera and screen, realSources=",
        realSources.size(),
        " reason=",
        reason
    );
    ...
    sendBridgeMessage(
        channel,
        constraintsMessage,
        "ReceiverVideoConstraints/v85a-live-safe-selected-1080p30-all-video/" + reason
    );
}
...
struct NativeWebRTCAnswerer::Impl {
    ...
    // v85: harder live guard. v84 still allowed a large enough backlog to create
    // multi-second NDI delay and big decoder bursts before the datachannel closed.
    // Keep only a tiny live window; stale video is cheaper to drop than to decode late.
    static constexpr std::size_t maxQueuedMediaPackets = 128;
    static constexpr std::size_t maxQueuedVideoPackets = 84;
    static constexpr std::size_t maxQueuedAudioPackets = 48;
    static constexpr std::size_t maxQueuedVideoPacketsPerSsrc = 18;
    static constexpr std::size_t targetQueuedVideoPacketsPerSsrc = 6;
    static constexpr int maxQueuedVideoAgeMs = 95;
    static constexpr int maxQueuedAudioAgeMs = 60;
...
NativeWebRTCAnswerer::NativeWebRTCAnswerer()
        ...
        for (;;) {
            ...
            if (packet.mid == "video" && queuedMs > Impl::maxQueuedVideoAgeMs) {
                ...
                if (dropped == 1 || (dropped % 100) == 0) {
                    Logger::warn(
                        "NativeWebRTCAnswerer: v85 dropping stale queued video RTP to keep latency low, dropped=",
                        dropped,
                        " queuedMs=",
                        queuedMs,
                        " ssrc=",
                ...
                if (dropped == 1 || (dropped % 100) == 0) {
                    Logger::warn(
                        "NativeWebRTCAnswerer: v85 dropping stale queued audio RTP to avoid delayed audio, dropped=",
                        dropped,
                        " queuedMs=",
                        queuedMs,
                        " ssrc=",
    ...
    Logger::info("NativeWebRTCAnswerer: v85a overload guard enabled; muted-source pruning, tighter live RTP queue, 1080p/30fps camera and screen-share constraints");
}
...
void NativeWebRTCAnswerer::resetSession() {
...

    {
        ...
        if (queued != 0) {
            Logger::warn("NativeWebRTCAnswerer: v85 cleared stale queued media packets on session reset, queued=", queued);
        }
...

            {
                ...
                for (auto it = impl_->mediaQueue.begin(); it != impl_->mediaQueue.end();) {
                    ...
                    if (staleVideo || staleAudio) {
                        ...
                        if (dropped == 1 || (dropped % 100) == 0) {
                            Logger::warn(
                                "NativeWebRTCAnswerer: v85 pruned stale queued RTP before enqueue, dropped=",
                                dropped,
                                " media=",
                ...
                if (mid == "video") {
                    while (rtpSsrc != 0 && countQueued("video", rtpSsrc) >= Impl::maxQueuedVideoPacketsPerSsrc) {
                        ...
                        if (dropped == 1 || (dropped % 100) == 0) {
                            Logger::warn(
                                "NativeWebRTCAnswerer: v85 dropping old video RTP for lagging SSRC to prevent one source adding latency, dropped=",
                                dropped,
                                " ssrc=",
                                rtpSsrc
                            );
                    ...
                    while (countQueued("video", 0) >= Impl::maxQueuedVideoPackets
                        ...
                        if (dropped == 1 || (dropped % 100) == 0) {
                            Logger::warn(
                                "NativeWebRTCAnswerer: v85 dropping old queued video RTP to keep live latency low, dropped=",
                                dropped,
                                " queued=",
                    ...
                    while (countQueued("audio", 0) >= Impl::maxQueuedAudioPackets) {
                        ...
                        if (dropped == 1 || (dropped % 100) == 0) {
                            Logger::warn(
                                "NativeWebRTCAnswerer: v85 dropping old queued audio RTP to avoid delayed audio, dropped=",
                                dropped,
                                " queued=",
                                impl_->mediaQueue.size()
                            );
                ...
                if (impl_->mediaQueue.size() >= Impl::maxQueuedMediaPackets) {
                    ...
                    if (dropped == 1 || (dropped % 100) == 0) {
                        Logger::warn(
                            "NativeWebRTCAnswerer: v85 media queue still full; dropping incoming RTP, media=",
                            mid,
                            " dropped=",
                            dropped,
                            " queued=",
