...
$backupDir = Join-Path $root ("backup_v85a_screen30_" + (Get-Date -Format "yyyyMMdd_HHmmss"))
New-Item -ItemType Directory -Force -Path $backupDir | Out-Null
Copy-Item -Force $target (Join-Path $backupDir "NativeWebRTCAnswerer.cpp")
Copy-Item -Force $patchFile $target

Write-Host "Applied v85a patch: screen-share remains 30fps. Backup: $backupDir"
