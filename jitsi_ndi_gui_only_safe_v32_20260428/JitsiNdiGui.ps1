...
function Start-FileLog {
    ...
    try {
        ...
        $safeRoom = Sanitize-Name $room
        if ([string]::IsNullOrWhiteSpace($safeRoom)) { $safeRoom = "room" }
        $stamp = Get-Date -Format "yyyyMMdd_HHmmss"
        $script:currentLogFile = Join-Path $logDir ("jitsi_ndi_gui_{0}_{1}.log" -f $stamp, $safeRoom)
        $script:logWriter = New-Object System.IO.StreamWriter($script:currentLogFile, $true, [System.Text.Encoding]::UTF8)
...
function Start-Receiver {
    ...
    # Keep native behavior as close as possible to the working version.
    # GUI may display quality/participants, but it must not pass unsupported flags like --quality.
    if ($chkPassNick.Checked) {
    ...
    if ($script:currentLogFile) { Append-Log ("[GUI] Log file: " + $script:currentLogFile) }
...
# Exe path is intentionally hidden from the main interface.
# It still can be changed through the small "Exe..." button if auto-detection is wrong.
$txtExe = New-Object System.Windows.Forms.TextBox
$txtExe.Text = Get-DefaultExePath
...
$chkPassNick = New-Object System.Windows.Forms.CheckBox
$chkPassNick.Text = "передавать ник в конференцию"
$chkPassNick.Dock = "Fill"
$chkPassNick.Checked = $true
$top.Controls.Add($chkPassNick, 2, 1)
$top.SetColumnSpan($chkPassNick, 2)
...
$btnBrowse.Add_Click({
    ...
    try {
        $dir = Split-Path $txtExe.Text -Parent
...
$top.Controls.Add($btnOpenLogs, 5, 1)

Add-Label "Качество:" 2 0 | Out-Null
...
[void]$cmbQuality.Items.Add("720p (не передаётся в native)")
[void]$cmbQuality.Items.Add("1080p (не передаётся в native)")
[void]$cmbQuality.Items.Add("2160p (не передаётся в native)")
$cmbQuality.SelectedIndex = 0
$top.Controls.Add($cmbQuality, 1, 2)

Add-Label "Фильтр участника:" 2 2 | Out-Null
$txtFilter = New-Object System.Windows.Forms.TextBox
$txtFilter.Dock = "Fill"
$txtFilter.Text = ""
$txtFilter.Add_TextChanged({
    $needle = $txtFilter.Text.Trim().ToLowerInvariant()
    ...
    foreach ($row in $grid.Rows) {
        ...
        try {
            if ([string]::IsNullOrWhiteSpace($needle)) {
                ...
                $hay = (([string]$row.Cells["Endpoint"].Value) + " " + ([string]$row.Cells["Name"].Value) + " " + ([string]$row.Cells["Kind"].Value)).ToLowerInvariant()
                $row.Visible = $hay.Contains($needle)
            }
        } catch {}
    }
})
$top.Controls.Add($txtFilter, 3, 2)
$top.SetColumnSpan($txtFilter, 3)
...
Append-Log "[GUI v32-safe-ui] Готово. Вставь ссылку Jitsi и нажми Старт."
Append-Log "[GUI] Основа запуска сохранена: GUI передает только --room и, если включено, --nick. --quality не передается."
