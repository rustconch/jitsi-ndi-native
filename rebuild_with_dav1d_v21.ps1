...
function Info($m) { Write-Host "[v21] $m" -ForegroundColor Cyan }
function Warn($m) { Write-Host "[v21][WARN] $m" -ForegroundColor Yellow }
function Die($m) { Write-Host "[v21][ERROR] $m" -ForegroundColor Red; exit 1 }

$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$ProjectRoot = (Get-Location).Path

if (-not (Test-Path (Join-Path $ProjectRoot 'CMakeLists.txt'))) {
  Die "Run this from the repository root, for example: cd D:\MEDIA\Desktop\jitsi-ndi-native"
}
...
Info "Repository root: $ProjectRoot"
...
# Plain ffmpeg:x64-windows is not enough for this case. We need ffmpeg built with the dav1d feature.
# --recurse is required because vcpkg must rebuild the already installed ffmpeg package with a new feature set.
& $vcpkgExe install --recurse --clean-after-build "dav1d:$triplet" "ffmpeg[core,avcodec,avdevice,avfilter,avformat,dav1d,opus,swresample,swscale,vpx]:$triplet" "openssl:$triplet"
...
# Make CMake re-detect and relink against updated vcpkg FFmpeg. Incremental build is not enough here.
...
$copyScript = Join-Path $ScriptDir 'copy_runtime_dlls_v21.ps1'
$checkScript = Join-Path $ScriptDir 'check_dav1d_runtime_v21.ps1'
if (-not (Test-Path $copyScript)) { Die "Helper script not found: $copyScript" }
if (-not (Test-Path $checkScript)) { Die "Helper script not found: $checkScript" }
