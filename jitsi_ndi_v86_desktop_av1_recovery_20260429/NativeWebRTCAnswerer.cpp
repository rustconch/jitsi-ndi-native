    ...
    */
    ...
    const auto now = std::chrono::steady_clock::now();
    {
        ...
        if (haveLastConstraints && constraintsMessage == lastConstraintsMessage) {
            ...
            if (sinceMs < 5000) {
                Logger::info(
                    "NativeWebRTCAnswerer: v86 skipped duplicate ReceiverVideoConstraints, reason=",
                    reason,
                    " sinceMs=",
                    sinceMs
                );
    ...
    Logger::info(
        "NativeWebRTCAnswerer: requesting v86 selected-only constraints: screen 1080p/30fps, camera 720p/30fps, realSources=",
        realSources.size(),
        " reason=",
        reason
    );
    ...
    sendBridgeMessage(
        channel,
        constraintsMessage,
        "ReceiverVideoConstraints/v86-screen1080p30-camera720p30/" + reason
    );
}
...
NativeWebRTCAnswerer::NativeWebRTCAnswerer()
    ...
    Logger::info("NativeWebRTCAnswerer: v86 desktop AV1 recovery enabled; screen-share 1080p/30fps, camera 720p/30fps, AV1-safe RTP queue");
}
