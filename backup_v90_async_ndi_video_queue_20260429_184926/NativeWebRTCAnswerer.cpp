...
std::string forceVp8OnlyVideoSdp(const std::string& sdp) {
    /*
        v89 VP8-stable receive mode.
        The v86-v88 AV1 path can keep audio perfect but destabilize video: after RTP
        gaps libdav1d/AV1 recovery may lag, and JVB can close the bridge datachannel.
        Force the synthetic offer and final answer to VP8 only so the bridge should
        allocate VP8 instead of PT=41/AV1. This keeps screen-share separate and at
        30fps; it only changes the received video codec.
    */
    std::vector<std::string> vp8PayloadTypes = extractVideoPayloadTypesByCodec(sdp, "VP8");
    if (vp8PayloadTypes.empty()) {
        // Jitsi commonly uses PT 100 for VP8; keep this fallback for synthetic SDP.
    ...
    while (std::getline(input, line)) {
        ...
        if (inVideo) {
            ...
            // AV1/SVC extensions are not needed in VP8-only receive mode.
    ...
    */
    ...
    const auto now = std::chrono::steady_clock::now();
    {
        ...
        if (haveLastConstraints && constraintsMessage == lastConstraintsMessage) {
            ...
            if (sinceMs < 5000) {
                Logger::info(
                    "NativeWebRTCAnswerer: v89 skipped duplicate ReceiverVideoConstraints, reason=",
                    reason,
                    " sinceMs=",
                    sinceMs
                );
    ...
    Logger::info(
        "NativeWebRTCAnswerer: requesting v89 VP8-only selected constraints: total video receive cap 6.7Mbps, screen 1080p/30fps, camera 720p/30fps, realSources=",
        realSources.size(),
        " reason=",
        reason
    );
    ...
    sendBridgeMessage(
        channel,
        constraintsMessage,
        "ReceiverVideoConstraints/v89-vp8only-cap6700kbps-screen1080p30-camera720p30/" + reason
    );
}
...
NativeWebRTCAnswerer::NativeWebRTCAnswerer()
    ...
    Logger::info("NativeWebRTCAnswerer: v89 VP8-only receive enabled; AV1 disabled in presence/session-accept/SDP, video cap 6.7Mbps, screen-share 1080p/30fps, camera 720p/30fps");
}
...
bool NativeWebRTCAnswerer::createAnswer(const JingleSession& session, Answer& outAnswer) {
    ...
    if (offerSdp != rawOfferSdp) {
        Logger::warn("NativeWebRTCAnswerer: v89 stripped AV1/VP9/H264 from remote offer; VP8-only receive mode active");
    }
    ...
    if (outAnswer.sdp != impl_->localSdp) {
        Logger::warn("NativeWebRTCAnswerer: v89 stripped AV1/VP9/H264 from local answer before Jingle session-accept");
    }
