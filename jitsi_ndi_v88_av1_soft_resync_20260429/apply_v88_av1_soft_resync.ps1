...
$backupDir = Join-Path $repoRoot ("backup_v88_av1_soft_resync_" + $stamp)
...
$restoreScript = Join-Path $backupDir "restore_v88_backup.ps1"
@"
`$ErrorActionPreference = "Stop"
...
Write-Host "Restored v88 backup from `$backupDir"
"@ | Set-Content -Encoding ASCII $restoreScript

Write-Host "Applied v88 AV1 soft-resync patch. Backup: $backupDir"
