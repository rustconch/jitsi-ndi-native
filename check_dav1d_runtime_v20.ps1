...
function Info($m) { Write-Host "[v20-check] $m" -ForegroundColor Cyan }
function Warn($m) { Write-Host "[v20-check][WARN] $m" -ForegroundColor Yellow }
function Die($m) { Write-Host "[v20-check][ERROR] $m" -ForegroundColor Red; exit 1 }
...
if (-not $davDll) {
  Die "No dav1d DLL beside exe. Run rebuild_with_dav1d_v20.ps1, not only cmake --build."
}
