...
std::vector<EncodedVideoFrame> Av1RtpFrameAssembler::flushCurrentFrame() {
    ...
    if (haveFragmentedObu_) {
        Logger::warn("Av1RtpFrameAssembler: marker/timestamp boundary while fragmented OBU is still open; dropping partial OBU");
        haveFragmentedObu_ = false;
        fragmentedObu_.clear();
    }
    ...
    /*
        If your EncodedVideoFrame field is called `bytes` instead of `data`,
...
bool Av1RtpFrameAssembler::appendAv1RtpPayload(
    ...
    */
    ...
    if (w == 0) {
        /*
            W == 0 means every OBU element is preceded by a LEB128 length.
        */
        while (pos < bodySize) {
        ...
        /*
            W == 1..3 means the packet contains W OBU elements.
            All except the last have LEB128 length.
            The last element length is inferred from remaining payload size.
        */
        for (int i = 0; i < w; ++i) {
...
bool Av1RtpFrameAssembler::appendObuElement(
    ...
    if (continuesPreviousObu) {
        if (!haveFragmentedObu_) {
            Logger::warn("Av1RtpFrameAssembler: got AV1 continuation fragment without previous fragment");
            return false;
        }
    ...
    if (haveFragmentedObu_) {
        Logger::warn("Av1RtpFrameAssembler: new AV1 OBU started while previous fragmented OBU is still open");
        haveFragmentedObu_ = false;
        fragmentedObu_.clear();
    }
...
bool Av1RtpFrameAssembler::appendCompletedObuForDecoder(
    ...
    /*
        ...
        RTP AV1 commonly carries OBUs with obu_has_size_field = 0.
        FFmpeg AV1 decoding is much happier if we reinsert OBU size fields.
    */
    ...
    /*
        Temporal delimiter OBU. RTP spec says receiver should ignore it.
    */
    if (obuType == 2) {
        return true;
    }
    ...
    /*
        Rebuild OBU as:
            modified_obu_header with obu_has_size_field = 1
