...
function Info($m) { Write-Host "[v19-check] $m" -ForegroundColor Cyan }
function Warn($m) { Write-Host "[v19-check][WARN] $m" -ForegroundColor Yellow }
function Die($m) { Write-Host "[v19-check][ERROR] $m" -ForegroundColor Red; exit 1 }
...
if (-not $davDll) {
  Die "No dav1d DLL beside exe. Run rebuild_with_dav1d_v19.ps1, not only cmake --build."
}
...
# Optional dumpbin check. If dumpbin is unavailable, the presence of dav1d DLL is still useful.
$roots = @(
