    ...
    */
    ...
    const auto now = std::chrono::steady_clock::now();
    {
        ...
        if (haveLastConstraints && constraintsMessage == lastConstraintsMessage) {
            ...
            if (sinceMs < 1500) {
                Logger::info(
                    "NativeWebRTCAnswerer: v84 skipped duplicate 1080p ReceiverVideoConstraints, reason=",
                    reason,
                    " sinceMs=",
                    sinceMs
                );
    ...
    Logger::info(
        "NativeWebRTCAnswerer: requesting v84 debounced selected-only 1080p/30fps constraints, realSources=",
        realSources.size(),
        " reason=",
        reason
    );
    ...
    sendBridgeMessage(
        channel,
        constraintsMessage,
        "ReceiverVideoConstraints/v84-debounced-selected-only-1080p30/" + reason
    );
}
...
struct NativeWebRTCAnswerer::Impl {
    ...
    // v84: live-first queue. v83 protected PeerConnection, but the queue could still
    // add noticeable delay when one camera/source decoded slowly. Keep only a short
    // live window and prefer dropping old video over increasing NDI latency.
    static constexpr std::size_t maxQueuedMediaPackets = 320;
    static constexpr std::size_t maxQueuedVideoPackets = 220;
    static constexpr std::size_t maxQueuedAudioPackets = 72;
    static constexpr std::size_t maxQueuedVideoPacketsPerSsrc = 56;
    static constexpr std::size_t targetQueuedVideoPacketsPerSsrc = 22;
    static constexpr int maxQueuedVideoAgeMs = 180;
    static constexpr int maxQueuedAudioAgeMs = 90;
...
NativeWebRTCAnswerer::NativeWebRTCAnswerer()
        ...
        for (;;) {
            ...
            if (packet.mid == "video" && queuedMs > Impl::maxQueuedVideoAgeMs) {
                ...
                if (dropped == 1 || (dropped % 100) == 0) {
                    Logger::warn(
                        "NativeWebRTCAnswerer: v84 dropping stale queued video RTP to keep latency low, dropped=",
                        dropped,
                        " queuedMs=",
                        queuedMs,
                        " ssrc=",
                ...
                if (dropped == 1 || (dropped % 100) == 0) {
                    Logger::warn(
                        "NativeWebRTCAnswerer: v84 dropping stale queued audio RTP to avoid delayed audio, dropped=",
                        dropped,
                        " queuedMs=",
                        queuedMs,
                        " ssrc=",
    ...
    Logger::info("NativeWebRTCAnswerer: v84 entry/latency optimized media worker enabled; short live RTP queue and debounced bridge subscriptions");
}
...
void NativeWebRTCAnswerer::resetSession() {
...

    {
        ...
        if (queued != 0) {
            Logger::warn("NativeWebRTCAnswerer: v84 cleared stale queued media packets on session reset, queued=", queued);
        }
    ...
    try {
        ...
        bridgeChannel->onMessage([
            ...
            if (text.find("\"colibriClass\":\"ForwardedSources\"") != std::string::npos) {
                ...
                if (sources.empty()) {
                    Logger::info(
                        "NativeWebRTCAnswerer: ForwardedSources is empty; v84 keeps the last known 1080p subscription instead of resending during join, knownSources=",
                        allKnownVideoSources.size()
                    );
...

            {
                ...
                for (auto it = impl_->mediaQueue.begin(); it != impl_->mediaQueue.end();) {
                    ...
                    if (staleVideo || staleAudio) {
                        ...
                        if (dropped == 1 || (dropped % 100) == 0) {
                            Logger::warn(
                                "NativeWebRTCAnswerer: v84 pruned stale queued RTP before enqueue, dropped=",
                                dropped,
                                " media=",
                ...
                if (mid == "video") {
                    while (rtpSsrc != 0 && countQueued("video", rtpSsrc) >= Impl::maxQueuedVideoPacketsPerSsrc) {
                        ...
                        if (dropped == 1 || (dropped % 100) == 0) {
                            Logger::warn(
                                "NativeWebRTCAnswerer: v84 dropping old video RTP for lagging SSRC to prevent one source adding latency, dropped=",
                                dropped,
                                " ssrc=",
                                rtpSsrc
                            );
                    ...
                    while (countQueued("video", 0) >= Impl::maxQueuedVideoPackets
                        ...
                        if (dropped == 1 || (dropped % 100) == 0) {
                            Logger::warn(
                                "NativeWebRTCAnswerer: v84 dropping old queued video RTP to keep live latency low, dropped=",
                                dropped,
                                " queued=",
                    ...
                    while (countQueued("audio", 0) >= Impl::maxQueuedAudioPackets) {
                        ...
                        if (dropped == 1 || (dropped % 100) == 0) {
                            Logger::warn(
                                "NativeWebRTCAnswerer: v84 dropping old queued audio RTP to avoid delayed audio, dropped=",
                                dropped,
                                " queued=",
                                impl_->mediaQueue.size()
                            );
                ...
                if (impl_->mediaQueue.size() >= Impl::maxQueuedMediaPackets) {
                    ...
                    if (dropped == 1 || (dropped % 100) == 0) {
                        Logger::warn(
                            "NativeWebRTCAnswerer: v84 media queue still full; dropping incoming RTP, media=",
                            mid,
                            " dropped=",
                            dropped,
                            " queued=",
