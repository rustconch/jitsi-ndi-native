...
struct NativeWebRTCAnswerer::Impl {
    ...
    static constexpr std::size_t maxQueuedMediaPackets = 4096;
...
NativeWebRTCAnswerer::NativeWebRTCAnswerer()
    ...
    Logger::info("NativeWebRTCAnswerer: v82 media worker queue enabled; WebRTC RTP callback will not decode/send NDI synchronously");
}
...
void NativeWebRTCAnswerer::resetSession() {
...

    {
        ...
        if (queued != 0) {
            Logger::warn("NativeWebRTCAnswerer: v82 cleared stale queued media packets on session reset, queued=", queued);
        }
        ...
        track->onMessage([this, mid](rtc::message_variant message) {
...

            {
                ...
                if (impl_->mediaQueue.size() >= Impl::maxQueuedMediaPackets) {
                    if (mid == "video") {
                        const auto dropped = ++impl_->droppedQueuedVideoPackets;
                        if (dropped == 1 || (dropped % 100) == 0) {
                            Logger::warn(
                                "NativeWebRTCAnswerer: v82 media queue full; dropping incoming video RTP before decode, dropped=",
                                dropped,
                                " queued=",
                    ...
                    auto videoIt = std::find_if(
                        impl_->mediaQueue.begin(),
                        impl_->mediaQueue.end(),
                        [](const Impl::QueuedMediaPacket& queued) {
                            return queued.mid == "video";
                        }
                    );

                    if (videoIt != impl_->mediaQueue.end()) {
                        impl_->mediaQueue.erase(videoIt);
                        const auto dropped = ++impl_->droppedQueuedVideoPackets;
                        if (dropped == 1 || (dropped % 100) == 0) {
                            Logger::warn(
                                "NativeWebRTCAnswerer: v82 media queue full; dropped queued video RTP to keep audio alive, dropped=",
                                dropped
                            );
                        }
                    } else {
                        const auto dropped = ++impl_->droppedQueuedAudioPackets;
                        if (dropped == 1 || (dropped % 100) == 0) {
                            Logger::warn(
                                "NativeWebRTCAnswerer: v82 media queue full; dropping incoming audio RTP, dropped=",
                                dropped
                            );
                        }
                        return;
