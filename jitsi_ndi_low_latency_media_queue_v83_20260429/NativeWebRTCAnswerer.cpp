...
struct NativeWebRTCAnswerer::Impl {
    ...
    // v83: keep this queue short. The goal is live NDI, not perfect packet archival.
    // A large queue protected PeerConnection in v82, but it also created seconds of
    // latency and A/V drift when one camera/source started decoding slowly.
    static constexpr std::size_t maxQueuedMediaPackets = 768;
    static constexpr std::size_t maxQueuedVideoPackets = 520;
    static constexpr std::size_t maxQueuedAudioPackets = 120;
    ...
    static constexpr int maxQueuedVideoAgeMs = 450;
    static constexpr int maxQueuedAudioAgeMs = 180;
...
NativeWebRTCAnswerer::NativeWebRTCAnswerer()
        ...
        for (;;) {
            ...
            if (packet.mid == "video" && queuedMs > Impl::maxQueuedVideoAgeMs) {
                ...
                if (dropped == 1 || (dropped % 100) == 0) {
                    Logger::warn(
                        "NativeWebRTCAnswerer: v83 dropping stale queued video RTP to keep latency low, dropped=",
                        dropped,
                        " queuedMs=",
                        queuedMs,
                        " ssrc=",
                ...
                if (dropped == 1 || (dropped % 100) == 0) {
                    Logger::warn(
                        "NativeWebRTCAnswerer: v83 dropping stale queued audio RTP to avoid delayed audio, dropped=",
                        dropped,
                        " queuedMs=",
                        queuedMs,
                        " ssrc=",
    ...
    Logger::info("NativeWebRTCAnswerer: v83 low-latency media worker enabled; bounded RTP queue with stale video/audio drop");
}
...
void NativeWebRTCAnswerer::resetSession() {
...

    {
        ...
        if (queued != 0) {
            Logger::warn("NativeWebRTCAnswerer: v83 cleared stale queued media packets on session reset, queued=", queued);
        }
...

            {
                ...
                for (auto it = impl_->mediaQueue.begin(); it != impl_->mediaQueue.end();) {
                    ...
                    if (staleVideo || staleAudio) {
                        ...
                        if (dropped == 1 || (dropped % 100) == 0) {
                            Logger::warn(
                                "NativeWebRTCAnswerer: v83 pruned stale queued RTP before enqueue, dropped=",
                                dropped,
                                " media=",
                ...
                if (mid == "video") {
                    while (rtpSsrc != 0 && countQueued("video", rtpSsrc) >= Impl::maxQueuedVideoPacketsPerSsrc) {
                        ...
                        if (dropped == 1 || (dropped % 100) == 0) {
                            Logger::warn(
                                "NativeWebRTCAnswerer: v83 dropping old video RTP for lagging SSRC to prevent one source adding latency, dropped=",
                                dropped,
                                " ssrc=",
                                rtpSsrc
                            );
                    ...
                    while (countQueued("video", 0) >= Impl::maxQueuedVideoPackets
                        ...
                        if (dropped == 1 || (dropped % 100) == 0) {
                            Logger::warn(
                                "NativeWebRTCAnswerer: v83 dropping old queued video RTP to keep live latency low, dropped=",
                                dropped,
                                " queued=",
                    ...
                    while (countQueued("audio", 0) >= Impl::maxQueuedAudioPackets) {
                        ...
                        if (dropped == 1 || (dropped % 100) == 0) {
                            Logger::warn(
                                "NativeWebRTCAnswerer: v83 dropping old queued audio RTP to avoid delayed audio, dropped=",
                                dropped,
                                " queued=",
                                impl_->mediaQueue.size()
                            );
                ...
                if (impl_->mediaQueue.size() >= Impl::maxQueuedMediaPackets) {
                    ...
                    if (dropped == 1 || (dropped % 100) == 0) {
                        Logger::warn(
                            "NativeWebRTCAnswerer: v83 media queue still full; dropping incoming RTP, media=",
                            mid,
                            " dropped=",
                            dropped,
                            " queued=",
