...
std::string makeReceiverVideoConstraintsMessage(
    ...
    /*
        ...
        - Desktop/screen-share sources are still requested as separate NDI sources, but at
          a lower frame rate. In the latest failing log two desktop sources plus two cameras
    ...
    */
    ...
    for (std::size_t i = 0; i < realSources.size(); ++i) {
        ...
        const double sourceMaxFrameRate = desktop ? 15.0 : 30.0;
...
void sendReceiverVideoConstraints(
    ...
    */
    ...
    Logger::info(
        "NativeWebRTCAnswerer: requesting v85 live-safe selected-only 1080p camera / 15fps screen constraints, realSources=",
        realSources.size(),
        " reason=",
        reason
    );
    ...
    sendBridgeMessage(
        channel,
        constraintsMessage,
        "ReceiverVideoConstraints/v85-live-safe-selected-1080p30-camera-1080p15-screen/" + reason
    );
}
...
NativeWebRTCAnswerer::NativeWebRTCAnswerer()
    ...
    Logger::info("NativeWebRTCAnswerer: v85 overload guard enabled; muted-source pruning, tighter live RTP queue, 1080p cameras and 15fps screen-share constraints");
}
