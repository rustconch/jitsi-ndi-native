...
// AV1_LOW_OVERHEAD_V86_GAP_RECOVERY
...
void Av1RtpFrameAssembler::markCorruptUntilMarker() {
    ...
    // v86: after an RTP sequence gap, do not feed dependent AV1 inter-frames
    // into dav1d. A single missing RTP packet can corrupt reference state and
    // cause repeated "Error parsing OBU data" until the next keyframe.
    decoderPrimed_ = false;
...
bool Av1RtpFrameAssembler::emitCurrentTemporalUnit(std::uint32_t timestamp, std::vector<EncodedVideoFrame>& out) {
    ...
    if (needKeyframeAfterGap_ && !currentUnitKey_) {
        ...
        if (droppedUntilSequenceHeader_ <= 10 || (droppedUntilSequenceHeader_ % 100) == 0) {
            Logger::warn(
                "Av1RtpFrameAssembler: v86 dropping dependent AV1 temporal unit after RTP gap until keyframe arrives. dropped=",
                droppedUntilSequenceHeader_
            );
        }
        return false;
