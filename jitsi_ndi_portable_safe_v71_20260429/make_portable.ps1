# Jitsi NDI Native - portable packager v71 safe no-watchdog
$ErrorActionPreference = 'Stop'
function Info($m) { Write-Host "[v71] $m" }
function Warn($m) { Write-Host "[v71][WARN] $m" -ForegroundColor Yellow }
function Fail($m) { Write-Host "[v71][ERROR] $m" -ForegroundColor Red; exit 1 }

$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$repoRoot = Split-Path -Parent $scriptDir
Set-Location $repoRoot

$templateGui = Join-Path $scriptDir 'JitsiNdiGui.PortableV71.ps1'
...
$portableRoot = Join-Path $distDir "JitsiNDI_Portable_v71_safe_$stamp"
...
Write-Host "Jitsi NDI Portable v71 check"
...
'@ | Set-Content -Encoding ASCII -Path (Join-Path $portableRoot 'README_PORTABLE_V71.txt')

$zipPath = Join-Path $distDir ("JitsiNDI_Portable_v71_safe_$stamp.zip")
