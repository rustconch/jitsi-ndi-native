...
function Write-Step($msg) { Write-Host "[v72] $msg" }
...
$backupRoot = Join-Path $repo ".jnn_patch_backups\stability_opt_v72_$stamp"
...
foreach ($f in $files) { Backup-File $f $backupRoot }

# 1) Router: reduce hot-path log spam and drop decoder burst backlog.
$routerPath = "src\PerParticipantNdiRouter.cpp"
...
$av1Path = "src\Av1RtpFrameAssembler.cpp"
...
$ffPath = "src\FfmpegMediaDecoder.cpp"
$ff = Read-Text $ffPath
