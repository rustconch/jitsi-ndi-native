    ...
    */
    ...
    const auto now = std::chrono::steady_clock::now();
    {
        ...
        if (haveLastConstraints && constraintsMessage == lastConstraintsMessage) {
            ...
            if (sinceMs < 5000) {
                Logger::info(
                    "NativeWebRTCAnswerer: v87 skipped duplicate ReceiverVideoConstraints, reason=",
                    reason,
                    " sinceMs=",
                    sinceMs
                );
    ...
    Logger::info(
        "NativeWebRTCAnswerer: requesting v87 selected-only constraints: total video receive cap 6.7Mbps, screen 1080p/30fps, camera 720p/30fps, realSources=",
        realSources.size(),
        " reason=",
        reason
    );
    ...
    sendBridgeMessage(
        channel,
        constraintsMessage,
        "ReceiverVideoConstraints/v87-cap6700kbps-screen1080p30-camera720p30/" + reason
    );
}
...
NativeWebRTCAnswerer::NativeWebRTCAnswerer()
    ...
    Logger::info("NativeWebRTCAnswerer: v87 receive bitrate cap enabled; video cap 6.7Mbps, screen-share 1080p/30fps, camera 720p/30fps, AV1-safe RTP queue");
}
