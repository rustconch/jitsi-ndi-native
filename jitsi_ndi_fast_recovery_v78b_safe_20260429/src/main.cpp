...
int runApp(int argc, char** argv) {
    ...
    try {
        ...
        // STABILITY_FAST_RECOVERY_V78B_SAFE:
        ...
        // subscriptions/constraints only when RTP counters actually stall.
        ...
        while (g_running) {
            ...
            if (recoveryHintsNow != lastRecoveryHints) {
                ...
                Logger::warn(
                    ...
                    delta,
                    "; v78b will not refresh immediately while RTP may still be flowing"
                );
            }
            ...
            const bool softRefreshCooldownPassed =
                (now - lastSoftRefresh > std::chrono::seconds(15));
            ...
            if (shouldSoftRefresh && softRefreshCooldownPassed) {
                ...
                Logger::warn(
                    "StabilityWatchdog: v78b soft refresh, reason=",
                    softRefreshReason,
                    " audioIdleSec=",
                    audioIdleSec,
                    " videoIdleSec=",
                ...
                if (!signaling.refreshReceiverSubscriptions(softRefreshReason)) {
                    Logger::warn(
                        "StabilityWatchdog: v78b soft refresh could not be sent; keeping session alive, no full reconnect"
                    );
                }
            }
            ...
            if (
                ...
                Logger::warn(
                    ...
                    "s video=",
                    videoIdleSec,
                    "s; v78b does not auto-disconnect because full reconnect was slow/unstable"
                );
            }
