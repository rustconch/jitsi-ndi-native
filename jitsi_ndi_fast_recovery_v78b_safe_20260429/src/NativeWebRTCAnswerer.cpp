...
std::string makeReceiverVideoConstraintsMessage(
    ...
    /*
        v78b safety rollback:
        keep the proven v42 selected-only 1080p constraint shape.

        Important: v78a changed selectedSources to an empty list and lowered the
        bandwidth assumption. In testing this correlated with heavy RTP sequence
        gaps and dav1d AV1 OBU parse errors. Do not change quality/subscription
        semantics inside a stability patch.
    */
...
void sendReceiverVideoConstraints(
    ...
    if (realSources.empty()) {
        Logger::warn(
            "NativeWebRTCAnswerer: v78b stable video constraints skipped because real video sources list is empty, reason=",
            reason
        );
        return;
    }
    ...
    Logger::info(
        "NativeWebRTCAnswerer: requesting v78b restored v42 selected-only 1080p/30fps constraints, realSources=",
        realSources.size(),
        " reason=",
        reason
    );
    ...
    sendBridgeMessage(
        ...
        "ReceiverVideoConstraints/v78b-v42-selected-only-1080p30/" + reason
    );
}
...
bool NativeWebRTCAnswerer::refreshReceiverSubscriptions(const std::string& reason) {
    ...
    if (!channel) {
        Logger::warn(
            "NativeWebRTCAnswerer: v78b soft refresh skipped; bridge datachannel unavailable, reason=",
            reason
        );
        return false;
    }
    ...
    if (!attempted) {
        Logger::warn(
            "NativeWebRTCAnswerer: v78b soft refresh skipped; no known audio/video sources, reason=",
            reason
        );
        return false;
    }
    ...
    Logger::info(
        "NativeWebRTCAnswerer: v78b soft refresh requested, videoSources=",
        videoSources.size(),
        " audioSources=",
        audioSources.size(),
        " reason=",
...
bool NativeWebRTCAnswerer::createAnswer(const JingleSession& session, Answer& outAnswer) {
    ...
    pc->onStateChange([this](rtc::PeerConnection::State state) {
        ...
        if (numericState == 3) {
            ++recoveryHints_;
            Logger::warn("NativeWebRTCAnswerer: v78b: WebRTC PeerConnection disconnected; recovery hint logged");
        }

        if (numericState == 4 || numericState == 5) {
            ++recoveryHints_;
            Logger::warn("NativeWebRTCAnswerer: v78b: WebRTC PeerConnection failed/closed; recovery hint logged");
        }
    });
    ...
    try {
        ...
        bridgeChannel->onOpen([
            ...
            Logger::info("NativeWebRTCAnswerer: v78b safe recovery mode; restored v42 1080p selected-only video request; endpoint-only audio; no delayed retry bursts");
        });
