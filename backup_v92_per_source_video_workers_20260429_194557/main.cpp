...
while (g_running) {
    ...
    std::string mediaFailureReason;
    const bool mediaFailureRequested = signaling.consumeMediaRecoveryRequest(&mediaFailureReason);

    const bool stallReconnectAllowed =
        sawRtpOnce &&
        now >= reconnectGraceUntil &&
        (now - lastPacketProgress > std::chrono::seconds(25)) &&
        (now - lastReconnectAttempt > std::chrono::seconds(45));

    const bool immediateReconnectAllowed =
        mediaFailureRequested &&
        (now - lastReconnectAttempt > std::chrono::seconds(8));

    const bool mayReconnect = immediateReconnectAllowed || stallReconnectAllowed;

    if (mediaFailureRequested && !immediateReconnectAllowed) {
        Logger::warn(
            "MediaRecoveryWatchdog: WebRTC failure signal ignored during reconnect cooldown, reason=",
            mediaFailureReason.empty() ? "?" : mediaFailureReason
        );
    }
    ...
    if (mayReconnect) {
        ...
        if (immediateReconnectAllowed) {
            Logger::warn(
                "MediaRecoveryWatchdog: WebRTC bridge failed; reconnecting Jitsi session immediately, reason=",
                mediaFailureReason.empty() ? "?" : mediaFailureReason,
                " attempt=",
                reconnectAttempt
            );
