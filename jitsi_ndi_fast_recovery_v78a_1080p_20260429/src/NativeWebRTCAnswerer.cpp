...
std::string makeReceiverVideoConstraintsMessage(
    ...
    /*
        v78a stable constraints:
        - Do not mark every participant as selected/on-stage. That can ask JVB to
          prioritize too many high layers at once and may destabilize the bridge.
        - lastN=-1 keeps all videos eligible for forwarding.
        - Per-source constraints keep all known real sources requested, but with a
          configured max height and a realistic bandwidth budget.
    */
    const std::vector<std::string> realSources = normalizeVideoSourceNamesForConstraints(sourceNames);
    const int lastN = -1;
    const std::vector<std::string> emptySelectedSources;
    const std::vector<std::string> emptyOnStageSources;

    const std::int64_t perSourceBudget =
        maxHeight >= 1080 ? 3500000LL : 1800000LL;
    const std::int64_t requestedBudget =
        static_cast<std::int64_t>(realSources.size()) * perSourceBudget;
    const std::int64_t assumedBandwidthBps =
        std::max<std::int64_t>(
            6000000LL,
            std::min<std::int64_t>(40000000LL, requestedBudget)
        );
    ...
    out << "\"selectedSources\":" << jsonStringArray(emptySelectedSources) << ",";
...
void sendReceiverVideoConstraints(
    ...
    const std::vector<std::string> realSources = normalizeVideoSourceNamesForConstraints(sourceNames);
    const int stableMaxHeight = 1080;

    if (realSources.empty()) {
        Logger::warn(
            "NativeWebRTCAnswerer: v78a stable video constraints skipped because real video sources list is empty, reason=",
            reason
        );
        return;
    }
    ...
    Logger::info(
        "NativeWebRTCAnswerer: requesting v78a stable all-sources 1080p/30fps constraints, realSources=",
        realSources.size(),
        " reason=",
        reason
    );
    ...
    sendBridgeMessage(
        channel,
        makeReceiverVideoConstraintsMessage(realSources, stableMaxHeight),
        "ReceiverVideoConstraints/v78a-stable-all-sources-1080p30/" + reason
    );
}
...
bool NativeWebRTCAnswerer::refreshReceiverSubscriptions(const std::string& reason) {
#if JNN_WITH_NATIVE_WEBRTC
    std::shared_ptr<rtc::DataChannel> channel;
...

    {
        std::lock_guard<std::mutex> lock(impl_->mutex);
        channel = impl_->bridgeChannel;
        videoSources = impl_->knownVideoSources;
        audioSources = impl_->knownAudioSources;
    }

    if (!channel) {
        Logger::warn(
            "NativeWebRTCAnswerer: v78 soft refresh skipped; bridge datachannel unavailable, reason=",
            reason
        );
        return false;
    }
    ...
    if (!audioSources.empty()) {
        sendReceiverAudioSubscriptionInclude(
            channel,
            audioSources,
            "soft-refresh-" + reason
        );
    ...
    if (!attempted) {
        Logger::warn(
            "NativeWebRTCAnswerer: v78 soft refresh skipped; no known audio/video sources, reason=",
            reason
        );
        return false;
    }
    ...
    Logger::info(
        "NativeWebRTCAnswerer: v78 soft refresh requested, videoSources=",
        videoSources.size(),
        " audioSources=",
        audioSources.size(),
        " reason=",
...
bool NativeWebRTCAnswerer::createAnswer(const JingleSession& session, Answer& outAnswer) {
    ...
    pc->onStateChange([this](rtc::PeerConnection::State state) {
        ...
        if (numericState == 3) {
            ++recoveryHints_;
            Logger::warn("NativeWebRTCAnswerer: v78: WebRTC PeerConnection disconnected; soft recovery hint raised");
        }

        if (numericState == 4 || numericState == 5) {
            ++recoveryHints_;
            Logger::warn("NativeWebRTCAnswerer: v78: WebRTC PeerConnection failed/closed; soft refresh may not be enough if the bridge closed media");
        }
    });
    ...
    try {
        ...
        bridgeChannel->onOpen([
            ...
            Logger::info("NativeWebRTCAnswerer: v78a fast recovery mode; keeping existing 1080p video request; endpoint-only audio; no delayed retry bursts");
        });

        bridgeChannel->onClosed([this]() {
            ++recoveryHints_;
            Logger::warn("NativeWebRTCAnswerer: bridge datachannel closed; soft recovery hint raised");
        });

        bridgeChannel->onError([this](std::string error) {
            ++recoveryHints_;
    ...
    pc->onTrack([this](std::shared_ptr<rtc::Track> track) {
        ...
        track->onClosed([this, mid]() {
            ++recoveryHints_;
            Logger::warn("NativeWebRTCAnswerer: remote track closed, mid=", mid, "; soft recovery hint raised");
        });
