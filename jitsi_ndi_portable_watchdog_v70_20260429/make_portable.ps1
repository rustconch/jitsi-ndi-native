# Jitsi NDI Native - portable packager v70 video watchdog
$ErrorActionPreference = 'Stop'
function Info($m) { Write-Host "[v70] $m" }
function Warn($m) { Write-Host "[v70][WARN] $m" -ForegroundColor Yellow }
function Fail($m) { Write-Host "[v70][ERROR] $m" -ForegroundColor Red; exit 1 }

$scriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$repoRoot = Split-Path -Parent $scriptDir
Set-Location $repoRoot

$templateGui = Join-Path $scriptDir 'JitsiNdiGui.PortableV70.ps1'
...
$portableRoot = Join-Path $distDir "JitsiNDI_Portable_v70_watchdog_$stamp"
...
Write-Host "Jitsi NDI Portable v70 check"
...
@'
Jitsi NDI Portable v70 watchdog

Main launch:
  JitsiNDI.exe

New in v70:
  - native console is hidden;
  - native stdout/stderr is written to logs\jitsi-ndi-native_YYYYMMDD_HHMMSS.log;
  - GUI does not display native logs live;
  - if NDI video frames stop for more than ~45 seconds after video already started, GUI restarts native automatically.

If video freezes again:
  1) run CHECK_PORTABLE_VIDEO.ps1;
  2) send the newest logs\jitsi-ndi-native_*.log around the freeze time.
'@ | Set-Content -Encoding ASCII -Path (Join-Path $portableRoot 'README_PORTABLE_V70.txt')

$zipPath = Join-Path $distDir ("JitsiNDI_Portable_v70_watchdog_$stamp.zip")
