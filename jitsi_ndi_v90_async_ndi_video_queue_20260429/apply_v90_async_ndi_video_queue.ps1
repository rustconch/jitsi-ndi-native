...
if (!(Test-Path $srcDir)) {
    throw "src directory not found. Extract this patch folder inside the jitsi-ndi-native repo and run the script from there."
}

$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$backupDir = Join-Path $repoRoot ("backup_v90_async_ndi_video_queue_" + $stamp)
...
$restoreScript = Join-Path $backupDir "restore_v90_backup.ps1"
@"
`$ErrorActionPreference = "Stop"
`$backupDir = Split-Path -Parent `$MyInvocation.MyCommand.Path
`$repoRoot = Resolve-Path (Join-Path `$backupDir "..")
`$srcDir = Join-Path `$repoRoot "src"
`$files = @("NativeWebRTCAnswerer.cpp", "Av1RtpFrameAssembler.cpp", "Av1RtpFrameAssembler.h", "NDISender.cpp", "NDISender.h")
foreach (`$file in `$files) {
...
Write-Host "Restored v90 backup from `$backupDir"
"@ | Set-Content -Encoding ASCII $restoreScript

Write-Host "Applied v90 async NDI video queue patch. Backup: $backupDir"
