# Jitsi NDI Native GUI v74d portable safe - no watchdog, hidden runner logging
# ASCII-only PowerShell script to avoid codepage/parser issues.
# Portable v74d: no watchdog/autorestart; native runs via hidden runner and writes native log file.
...
function Start-NativeFromUi {
    try {
        if ($script:proc -and -not $script:proc.HasExited) {
            Append-Log '[GUI] Native runner is already running.'
            return
        }
        ...
        $runnerFile = Join-Path $script:logDir "run_native_$stamp.ps1"
        ...
        $argLiterals = @()
        foreach ($a in $argsList) { $argLiterals += (Quote-PsString ([string]$a)) }
        $argArray = '@(' + ($argLiterals -join ', ') + ')'
        $runner = @"
`$ErrorActionPreference = 'Continue'
`$env:PATH = $(Quote-PsString $releaseDir) + ';' + $(Quote-PsString $script:repoRoot) + ';' + `$env:PATH
Set-Location -LiteralPath $(Quote-PsString $releaseDir)
'# native log redirected by portable GUI v74d' | Out-File -LiteralPath $(Quote-PsString $script:nativeLogFile) -Encoding utf8
('started: ' + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss.fff')) | Out-File -LiteralPath $(Quote-PsString $script:nativeLogFile) -Append -Encoding utf8
`$nativeArgs = $argArray
& $(Quote-PsString $exe) @nativeArgs *>> $(Quote-PsString $script:nativeLogFile)
`$ec = `$LASTEXITCODE
('exited: ' + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss.fff') + ' code=' + `$ec) | Out-File -LiteralPath $(Quote-PsString $script:nativeLogFile) -Append -Encoding utf8
exit `$ec
"@
        ...
        $psi.FileName = $psExe
        $psi.Arguments = '-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File ' + (Quote-Arg $runnerFile)
        $psi.WorkingDirectory = $script:repoRoot
        $psi.UseShellExecute = $false
        $psi.RedirectStandardOutput = $false
        $psi.RedirectStandardError = $false
        $psi.CreateNoWindow = $true
        try { $psi.EnvironmentVariables['PATH'] = $releaseDir + ';' + $script:repoRoot + ';' + $psi.EnvironmentVariables['PATH'] } catch {}
        ...
        $script:isStopping = $false
        Set-RunningUi $true
        Append-Log "[GUI] Started hidden native runner. PID=$($p.Id)"
...
function Stop-NativeProcess {
    ...
    try {
        ...
        if ($script:proc -and -not $script:proc.HasExited) {
            Append-Log "[GUI] Stopping native runner/process tree... $Reason"
            try { Stop-ProcessTreeSafe $script:proc.Id } catch {}
...
$btnOpenLog.Add_Click({
    try {
        if ($script:nativeLogFile -and (Test-Path $script:nativeLogFile)) {
            Start-Process notepad.exe $script:nativeLogFile
        } elseif ($script:currentLogFile -and (Test-Path $script:currentLogFile)) {
            Start-Process notepad.exe $script:currentLogFile
        } else { Append-Log '[GUI] No log file yet.' }
    ...
    try {
        if ($script:proc) {
            if ($script:proc.HasExited) {
                if (-not $script:isStopping) { Append-Log "[GUI] Native runner exited with code $($script:proc.ExitCode)." }
                $script:proc = $null
                Set-RunningUi $false
                Update-VisualLayout
            } else {
